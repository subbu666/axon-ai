...
export default function Pricing() {
  ...
  const movePill = useCallback((targetRef) => {
    ...
    pill.style.transform = `translateX(${btnRect.left - containerRect.left - 4}px)`;
  }, []);

  // Position pill on mount + billing change
  useEffect(() => {
    movePill(billing === "monthly" ? monthlyBtnRef : annualBtnRef);
  }, [billing, movePill]);
  ...
  useEffect(() => {
    ...
    if (monthlyBtnRef.current) ro.observe(monthlyBtnRef.current.parentElement);
  ...
  const updatePriceNodes = useCallback((newBilling, newCurrency) => {
    TIER_KEYS.forEach((key) => {
      ...
      if (priceRefs.current[key])
        priceRefs.current[key].textContent = result.amount;
      if (symbolRefs.current[key])
        symbolRefs.current[key].textContent = result.symbol;
      if (savingsRefs.current[key]) {
        savingsRefs.current[key].textContent =
          result.savings > 0 ? `Save ${result.symbol}${result.savings}/yr` : "";
      }
    });
  }, []);

  useEffect(() => {
    updatePriceNodes(billing, currency);
  }, [billing, currency, updatePriceNodes]);
  ...
  return (
    <section aria-label="pricing" id="pricing" className="pricing-section">
      ...
      <div className="section-container">
        <h2 className="pricing-title" data-reveal>
          Simple Pricing
        </h2>
        ...
        <div className="billing-toggle" role="group" aria-label="Billing cycle">
          {/* Sliding pill — absolutely positioned inside the toggle */}
          <span ref={pillRef} className="billing-pill" aria-hidden="true" />
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <span
                    ref={(el) => (priceRefs.current[key] = el)}
                ...
                <button
                  className={`tier-cta ${tier.highlighted ? "cta-primary" : "cta-secondary"}`}
                >
                  {tier.ctaLabel}
                </button>
              </article>
